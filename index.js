...
bot.start(async (ctx) => {
  ...
  cron.schedule('* * 10 Jan,Feb,Mar,Apr,May,June,July,Aug,Sept,Oct,Nov,Dec Sun', () => {
    ctx.reply(`Привет, ${
      ...
      `,
      console.log('Каждый  месяц первого числа в 10 часов утра реклама'),
...
bot.hears("🏗Строительные леса🏗", async (ctx) => {
  try {
    const response = await axios.get('https://xn--80aagge2ckkol0hd.xn--p1ai/api/price_scaff');
    const data = response.data;
    
    await ctx.replyWithPhoto(data.price.path);
    ...
    await ctx.replyWithDocument({source:`${data.price.filepath}`});
  } catch (e) {
    console.error(e);
  }
});
...
bot.hears("🏗Вышки-туры🏗", async (ctx) => {
  try {
    const response = await axios.get('https://xn--80aagge2ckkol0hd.xn--p1ai/api/price_tours');
    const data = response.data;

    ctx.replyWithPhoto(data.price.path);
    await ctx.replyWithHTML(
